...
public class Game {
	...
	private int selectionState = 0;// 0 not selecting; 1 selecting the first square; 2 selecting the second square;
									// 3 area selected
	...
	public void saveToBeInserted(File file) throws FileNotFoundException, FileAlreadyExistsException {
		...
		try {
			if (file.createNewFile()) {
				...
				throw new FileAlreadyExistsException(file.getName(), null,
						"The file couldn't be stored because a file with that exact name already exists.");
			}
	...
	public boolean stepBack() {// Return whether there was a past state or not to be restored
		if (past.size() > 0) {
			while (past.size() > 1 && equalsBoard(past.getLast(), board)) {// While the past is the same as the
																			// present...
	...
	private void storePresent() {
		if (past.size() == 0 || !equalsBoard(board, past.getLast()))// If the board has changed from the last one...
																	// (otherwise don't save the same board two times)
		{
